...
contract ReflectingToken is ERC20, Ownable {
    ...
    // FIX: New mapping to handle tax exemptions correctly.
        ...
        Ownable(msg.sender)
    {
        ...
        // FIX: Setup tax exemptions
        _isExcludedFromTax[deployer] = true;
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        // This logic correctly triggers the swap only on qualifying transactions (like a user selling)
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        // FIX: Corrected tax logic to apply tax to all non-exempt transactions, including buys/sells.
        if (taxFeePercent > 0 && !_isExcludedFromTax[from] && !_isExcludedFromTax[to]) {
    ...
    function _removeHolder(address holder) private {
        ...
        if (index >= _holders.length) return;
    ...
    function manualSwapAndDistribute() external onlyOwner {
        uint256 contractTokenBalance = balanceOf(address(this));
        require(contractTokenBalance >= swapThreshold, "Swap threshold not met.");
        _swapAndDistributeDividends(contractTokenBalance);
    }
    ...
    // FIX: New function to manage tax exclusions
    function setExcludedFromTax(address account, bool isExcluded) external onlyOwner {
